...
{
	return (Base) + (GetTypeHash<QWORD>(Frame) << 8);
}

inline QWORD MakeCacheID( ECacheIDBase Base, UObject* Obj, QWORD DWord )
{
	return (Base) + (GetTypeHash<DWORD>(Obj) << 8) + (DWord << 32);
}

inline QWORD MakeCacheID( ECacheIDBase Base, UObject* Obj, UObject* Frame )
{
	return (Base) + (GetTypeHash<DWORD>(Obj) << 8) + (GetTypeHash<QWORD>(Frame) << 32);
}

inline QWORD MakeCacheID( ECacheIDBase Base, DWORD Word, DWORD Byte, UObject* Frame )
{
	return (Base) + (Byte<<8) + (Word<<16) + (GetTypeHash<QWORD>(Frame) << 32);
}

inline QWORD MakeCacheID( ECacheIDBase Base, DWORD ByteA, DWORD ByteB, DWORD ByteC, UObject* Frame )
{
	return (Base) + (ByteA<<8) + (ByteB<<16) + (ByteC<<24) + (GetTypeHash<QWORD>(Frame) << 32);
}

...
{
	return FColor(92,92,96,255);
}
FColor ColorClass::DarkGray()
{
	return FColor(32,32,36,255);
}
...
{
	GC->pCanvasUtil->DrawTile(PosX, PosY, PosX+Width, PosY+Height, 0.0,0.0,92.0,92.0,0.0,(UMaterial*)WhiteTexture,Color);
}

void Drawings::DrawTransparent(float PosX, float PosY, float Width, float Height, FColor Color)
{
	WhiteTexture->bAlphaTexture = true;

	GC->pCanvasUtil->DrawTile(PosX, PosY, PosX+Width, PosY+Height, 0.0,0.0,92.0,92.0,0.0,(UMaterial*)WhiteTexture,FColor(92,92,96,200));//Color);

	//WhiteTexture->bAlphaTexture = false;
}

void Drawings::Print(float PosX, float PosY, FColor Color, TCHAR * Text)
{
	FColor TempColor = GC->DrawColor;

	GC->DrawColor = Color;
	GC->pCanvasUtil->DrawString(PosX,PosY,(const unsigned short *)Text,SmallFont, GC->DrawColor,0);

	GC->DrawColor = TempColor;
}
...
void Drawings::CreateNewWindow(float PosX, float PosY, float Width, float Height, int Style, bool Translucent, TCHAR*Title)
{
	//OutsideBox
...
void Drawings::CreateNewButton(float PosX, float PosY, float Width, float Height, bool bDown, bool Translucent, TCHAR*Title)
{
	if(!Translucent)
	{
		if(!bDown)
...
void Drawings::CreateNewMouseCursor (float &PosX, float &PosY)
{
	...
	GC->pCanvasUtil->DrawTile(PosX,PosY,PosX+32,PosY+32, 55.0,62.0,92.0,92.0,0.0,(UMaterial*)MouseTexture,Color.White());
}
...
void MouseCheck::UpdateMouse(FLOAT DeltaTime)
{
	if(bMenuVisible)
	{
		MyMouseX += PC->aMouseX * 0.01;
		MyMouseY -= PC->aMouseY * 0.01;
		PC->aMouseX = 0;
		PC->aMouseY = 0;
	}
}
...
void UnrealWindow::DrawWindow(MorbidWindow Window, float PositionX,float PositionY,float Width,float Height, int Style, bool Translucent, TCHAR*Title)
{
...
void UnrealWindow::AddCheckBox(float PositionX, float PositionY, bool Option, bool bRightAlinedText, TCHAR * BoxName)
{
	if(bRightAlinedText)
	{
...
void UnrealWindow::AddButton(float PositionX, float PositionY,float Width,float Height, TCHAR * ButtonName,bool Transparent)
{
...
void UnrealWindow::Init()
{
	if(!bInit)
	{
		...
		WhiteTexture	= MWindow.InitializeTexture(TEXT("Engine.WhiteTexture"));
		MouseTexture	= MWindow.InitializeTexture(TEXT("2K4Menus.Cursors.Pointer"));
		SmallFont		= MWindow.InitializeFont(TEXT("Engine.DefaultFont"));
	}
}

void UnrealWindow::MakeShadowText(float PosX, float PosY, TCHAR * Text, FColor FontColor,UFont * Font)
{

	FColor TempColor = GC->DrawColor;
	GC->DrawColor = Color.Black();

	GC->pCanvasUtil->DrawString(PosX-1,PosY,(const unsigned short *)Text,Font, GC->DrawColor,0);

	GC->pCanvasUtil->DrawString(PosX+1,PosY,(const unsigned short *)Text,Font, GC->DrawColor,0);

	GC->pCanvasUtil->DrawString(PosX,PosY-1,(const unsigned short *)Text,Font, GC->DrawColor,0);

	GC->pCanvasUtil->DrawString(PosX,PosY+1,(const unsigned short *)Text,Font, GC->DrawColor,0);

	GC->DrawColor = FontColor;
	GC->pCanvasUtil->DrawString(PosX,PosY,(const unsigned short *)Text,Font, GC->DrawColor,0);
	GC->DrawColor = TempColor;
}
...
{
	UViewport* ViewPort = (UViewport*)PC->Player;
